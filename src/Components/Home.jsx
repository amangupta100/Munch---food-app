        ...
        <div className="px-16 grid grid-cols-4 lm:flex lm:flex-col lm:items-center lD:grid-cols-3 tb:grid-cols-2 gap-8 my-14">
           {FoodData.map((elem)=>{
              return(
                ...
                <div key={elem.id} className="lm:h-[380px] relative divCont lm:w-[100%] rounded-2xl bg-slate-100 hover:shadow-xl hover:shadow-gray-400 duration-200 p-3">
                    
                    <img src={elem.img} className="w-full  duration-150 rounded-2xl lm:h-[150px] h-[180px]" alt="" />
              <div className="flex flex-col">
                <div className="flex my-3 justify-between">
                    <h1 className="text-lg font-bold"> {elem.name} </h1>
                    <h1 className="font-semibold text-lg text-green-400">₹{elem.price} </h1>
                </div>
                <p className="lm:text-[16px]"> {elem.desc.slice(0,30)}...   <NavLink to={`${elem.id}`} target="_self" className="text-blue-400"><button onClick={()=>clsetElem(elem)}> Read More </button></NavLink></p>
                <div className="flex justify-between my-3 lm:my-3 items-center">
                   <section className="flex items-center gap-1"><FaStar className="text-yellow-400 text-xl" /> <h1 className="text-[20px] lm:text-[16px] font-bold inline-block">  {elem.rating} </h1></section>
